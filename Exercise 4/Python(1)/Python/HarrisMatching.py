...
distmat_ncc = np.zeros((npts1, npts2))
for i1 in range(npts1):
    for i2 in range(npts2):
        patch1 = patches1[:,:,i1]
        patch2 = patches2[:,:,i2]
        ncc_value = np.sum((patch1 - np.mean(patch1)) * (patch2 - np.mean(patch2))) / \
                    (np.sum((patch1 - np.mean(patch1)) ** 2) * np.sum((patch2 - np.mean(patch2)) ** 2)) ** 0.5
        distmat_ncc[i1, i2] = ncc_value

ss1_ncc = np.amax(distmat_ncc, axis=1)
ids1_ncc = np.argmax(distmat_ncc, axis=1)
ss2_ncc = np.amax(distmat_ncc, axis=0)
ids2_ncc = np.argmax(distmat_ncc, axis=0)

pairs = []
for k in range(npts1):
    if k == ids2_ncc[ids1_ncc[k]]:
        pairs.append(np.array([k, ids1_ncc[k], ss1_ncc[k]]))
pairs = np.array(pairs)
